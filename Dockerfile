...
ENTRYPOINT /sysconfcpus/bin/sysconfcpus -n 2 elm-make --yes --warn src/Main.elm --output shipbuilder/js/shipbuilder.js 2> msg || true \
        && if grep -q "Detected errors" msg; then cat msg && rm msg && exit -1; fi \
        && if grep -q "ERROR" msg; then cat msg && rm msg && exit -1; fi || rm msg \
#        && /sysconfcpus/bin/sysconfcpus -n 2 elm-test --skip-install 2> msg || true \
        && touch msg && cat msg \
        && if grep -q "fail" msg; then cat msg && rm msg && exit -1; fi \
        && if grep -q "TEST RUN FAILED" msg; then cat msg && rm msg && exit -1; fi \
        && if grep -q "Detected errors" msg; then cat msg && rm msg && exit -1; fi \
        && if grep -q "ERROR" msg; then cat msg && exit -1; fi \
        && rm msg
